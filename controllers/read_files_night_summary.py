...
for i in files_list:
    ...
    for j in lines:
        ...
        object_to_insert.date_fichero = CleanerFiles.get_date_from_ns_files(i)
        object_to_insert.registration_date = clean_list[3]
...
for object_aux in list_object_to_insert:
    ...
    run_aux.hour = object_aux.hora
    run_aux.id_date = dates_service.get_date_by_id(date_dto.date_dto).id_date
    run_aux.init_time_collect_data =  object_aux.registration_date
    run_aux.id_run_type = [runtype.id_run_type for runtype in run_type_service.get_all_run_type() if
                           runtype.description_run_type == object_aux.run_type]
    if len(run_aux.id_run_type) != 0:
        run_aux.id_run_type = run_aux.id_run_type[0]
    run_aux.id_config = [run_config.id_config for run_config in config_service.get_all_config() if
                         run_config.config_description == 'LSTCam-003']
    ...
    for run in runs_list:
        if run.id_date == run_aux.id_date and run.id_config == run_aux.id_config and run.id_run_type == run_aux.id_run_type\
                and run.run_number == int(run_aux.run_number):
            flag_run = True
            break
    if not flag_run:
